...
--"label"
--"text"
--"syntax_mode" : "euphoria", "creole", "html", "css", "xml", "ini", "c"
--"view_mode" : "raw", "syntax", "formatted", "hex"
--"edit_mode" : "normal", "block", "table"
--"line_numbers"
--"locked"
...
iTokenStyles = {},      --syntax highlighting styles (iViewMode=vmSyntax)
iSyntaxFont = {},       --font name to use when iViewMode=vmSyntax
iSyntaxSize = {},       --font size to use when iViewMode=vmSyntax
iFormatStyles = {},     --formatted text styles (iViewMode=vmFormatted)
...
enum            --iViewMode - view modes
vmRaw,              --Raw text
vmSyntax,           --Syntax highlighted text (use iTokenStyles, monowidth font)
vmFormatted,        --Render to formatted text (use iFormatStyles, variable fonts, font sizes, line spacing, paragraph spacing)
vmHex               --Hexidecimal codes
...
ttWhiteSpace,       --spaces or tabs (appears as special symbols if "show whitespace" option is enabled)
...
ttOpenSymb,         --synEuphoria: ({[ (tokenInfo specifices color/how many levels deep)
ttCloseSymb,        --synEuphoria: )}] (tokenInfo specifices color/how many levels deep)
ttOpenQuote,        --"'
ttCloseQuote,       --"'
ttQuoteText,        --text inside quotes that isn't whitespace

ttLineComment,      --Line Comment
ttOpenComment,      --Start multi-line comment
ttCloseComment,     --End multi-line comment
ttCommentText,      --text inside a comment that isn't whitespace

ttFormatted         --formatted text (tokenInfo specifices which style to use)
...
constant euKeywords = {"if", "elsif", "else","then", "end", "while", "for"}   --keywords built-in to euphoria

sequence
euIdentifiers = {}, --words recognised as declared identifiers (from analyzing existing source)
ttStyles = repeat({}, ttFormatted) --token styles: {textfont, textsize, textstyle, textcolor}
...
constant
headingheight = 16,
thCurrLineBkColor = rgb(255, 255, 200),
...
ttStyles[ttWhiteSpace] = {thMonoFont, thMonoFontSize, Normal, th:cButtonLabel}

ttStyles[ttIdentifier] = {thMonoFont, thMonoFontSize, Normal, rgb(0, 0, 128)}
ttStyles[ttKeyword] = {thMonoFont, thMonoFontSize, Normal, rgb(0, 128, 128)}
ttStyles[ttNumber] = {thMonoFont, thMonoFontSize, Normal, rgb(128, 0, 128)}
ttStyles[ttSymbol] = {thMonoFont, thMonoFontSize, Normal, rgb(128, 0, 0)}

ttStyles[ttOpenSymb] = {thMonoFont, thMonoFontSize, Normal, th:cButtonLabel}
ttStyles[ttCloseSymb] = {thMonoFont, thMonoFontSize, Normal, th:cButtonLabel}
ttStyles[ttOpenQuote] = {thMonoFont, thMonoFontSize, Normal, th:cButtonLabel}
ttStyles[ttCloseQuote] = {thMonoFont, thMonoFontSize, Normal, th:cButtonLabel}
ttStyles[ttQuoteText] = {thMonoFont, thMonoFontSize, Normal, th:cButtonLabel}

ttStyles[ttLineComment] = {thMonoFont, thMonoFontSize, Normal, th:cButtonLabel}
ttStyles[ttOpenComment] = {thMonoFont, thMonoFontSize, Normal, th:cButtonLabel}
ttStyles[ttCloseComment] = {thMonoFont, thMonoFontSize, Normal, th:cButtonLabel}
ttStyles[ttCommentText] = {thMonoFont, thMonoFontSize, Normal, th:cButtonLabel}

ttStyles[ttFormatted] = {thMonoFont, thMonoFontSize, Normal, th:cButtonLabel}
...
function parse_syntax(atom idx, sequence txt)
    ...
    if iViewMode[idx] = vmSyntax then
...
*/
                ...
                for t = 1 to length(ttexts)-1 do
                    if isComment then
                        ttypes[t] = ttStyles[ttCommentText]
                        
                    elsif isQuote then
                        ttypes[t] = ttStyles[ttQuoteText]
    ...
    /*    
    elsif iViewMode[idx] = vmFormatted then
        switch iSyntaxMode[idx] do
            case synCreole then
                ttexts = split(txt)
                if length(ttexts) > 1 then
    ...
    elsif iViewMode[idx] = vmHex then
    */
            ...
            if length(iCmdQueue[idx]) > 0 then
                ...
                switch iCmdQueue[idx][1][1] do
                    case cmdDestroyEditor then --iCmdQueue[idx][1][2] = {}
                        ...
                        iViewMode[idx]      = remove(iViewMode[idx], idx)
                        ...
                        iFormatStyles[idx]  = remove(iFormatStyles[idx], idx)
...
export procedure create(sequence wprops)
    ...
    nTokenStyles    = {     --each token type: {tsBackColor, tsTextColor, tsBold, tsItalic, tsUnderline}
        ...
        {thBackColor, rgb(0, 0, 0), 0, 0, 0},  --ttWhiteSpace
        ...
        {thBackColor, rgb(0, 0, 0), 0, 0, 0},  --ttOpenSymb
        {thBackColor, rgb(0, 0, 0), 0, 0, 0},  --ttCloseSymb
        {thBackColor, rgb(0, 0, 0), 0, 0, 0},  --ttOpenQuote
        {thBackColor, rgb(0, 0, 0), 0, 0, 0},  --ttCloseQuote
        {thBackColor, rgb(0, 0, 0), 0, 0, 0},  --ttQuoteText
        
        {thBackColor, rgb(0, 0, 0), 0, 0, 0},  --ttLineComment
        {thBackColor, rgb(0, 0, 0), 0, 0, 0},  --ttOpenComment
        {thBackColor, rgb(0, 0, 0), 0, 0, 0},  --ttCloseComment
        {thBackColor, rgb(0, 0, 0), 0, 0, 0},  --ttCommentText
        
        {thBackColor, rgb(0, 0, 0), 0, 0, 0}   --ttFormatted
    },
    ...
    for p = 1 to length(wprops) do
        if length(wprops[p]) = 2 then
            switch wprops[p][1] do         
                ...
                case "view_mode" then
                    if equal(text:lower(wprops[p][2]), "raw") then
                        nViewMode = vmRaw
                    elsif equal(text:lower(wprops[p][2]), "syntax") then
                        nViewMode = vmSyntax
                    elsif equal(text:lower(wprops[p][2]), "formatted") then
                        nViewMode = vmFormatted
                    elsif equal(text:lower(wprops[p][2]), "hex") then
                        nViewMode = vmHex 
                    end if
                ...
                case "format_styles" then
                    nFormatStyles = wprops[p][2]*/
                ...
                if py + iParaSize[idx][p][2] >= vtop and py < vbottom then
                    ...
                    for t = 1 to length(tokens[tokenText]) do
                        if py + tokens[tokenPos][t][2]+tokens[tokenSize][t][2] >= vtop and py + tokens[tokenPos][t][2] < vbottom then
                            ...
                            ? p
                            ? t
                            puts(1, tokens[tokenText][t])
                            ? tokens[tokenType][t]
                            ? ttStyles[tokens[tokenType][t]]*/
