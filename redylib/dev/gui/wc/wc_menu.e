...
itmTextPos,
itmSubMenu,
itmType   --0=pinbutton, 1=normal, 2=separator, 3=submenu
...
atom thPinButtonHeight = 12, showpin = 0
...
procedure wc_create(atom wid, object wprops) 
    ...
    sequence wsize = {6, 6}, txex, itms = {}, mitems = {{}, {}, {}, {}, {}}, avrect = {0, 0, 0, 0, 0}, wtitle = ""
    ...
    for m = 1 to length(itms) do            
        if equal(itms[m], "-") then --separator
            txex = {0, 0, 0, 0, 0, 0}
            mitems[itmLabel] &= {"-"}
            mitems[itmType] &= 2
            mitems[itmSubMenu] &= {{}}
        elsif sequence(itms[m][1]) then --submenu
            mitems[itmLabel] &= {itms[m][1]} --& " >"
            mitems[itmType] &= 3
            mitems[itmSubMenu] &= {itms[m][2]}
            if find('*', itms[m][1]) = 1 then  --disabled item
                txex = oswin:get_text_extent(winhandle, itms[m][1][2..$])
            else
                txex = oswin:get_text_extent(winhandle, itms[m][1])
            end if
        else --normal
            mitems[itmLabel] &= {itms[m]}
            mitems[itmType] &= 1
            mitems[itmSubMenu] &= {{}}
            if find('*', itms[m]) = 1 then  --disabled item
                txex = oswin:get_text_extent(winhandle, itms[m][2..$])
            else
                txex = oswin:get_text_extent(winhandle, itms[m])
            end if
        end if
...
procedure wc_draw(atom wid)
    ...
    atom idx, itmh, hlcolor, shcolor, fillcolor, txtcolor, chkcolor, x1, y1, itmdisabled
    idx = find(wid, wcprops[wcpID])
    ...
    if idx > 0 then
        ...
        for m = 1 to length(mitems[itmLabel]) do            
            ...
            itmdisabled = (find('*', mitems[itmLabel][m]) = 1)
            if wcprops[wcpSelection][idx] = m and mitems[itmType][m] != 2 and itmdisabled = 0 then
                chkcolor = th:cInnerSel
                txtcolor = th:cInnerTextSel
            else
                if wcprops[wcpSoftFocus][idx] = m and mitems[itmType][m] != 2 and itmdisabled = 0 then
                    chkcolor = th:cInnerSel
                    txtcolor = th:cInnerTextSel
                else
            ...
            elsif mitems[itmType][m] = 1 then --normal
                --label:
            ...
            case "MouseMove" then --{x, y, shift, mousepos[1], mousepos[2]}
                ...
                for m = 1 to length(mitems[itmLabel]) do
                    if in_rect(evdata[1], evdata[2], mitems[itmRect][m]) then
                        ...
                        --if find('*', mitems[itmLabel][m]) = 0 then
...
/*
        ...
        if sf != wcprops[wcpSoftFocus][idx] then
            ...
            if sf > 0 then
                ...
                if sf > 0 and mitems[itmType][sf] = 3 and find('*', mitems[itmLabel][sf]) = 0 then --if submenu and not disabled, then show submenu
        ...
        if clicked > 0 then
            if mitems[itmType][clicked] = 0 then  --Pin item
                if wcprops[wcpPinned][idx] = 0 then
                    ...
                    winhandle = oswin:create_window(wid, wtitle, "tool", wpos[1], wpos[2], wsize[1], wsize[2], th:cOuterFill)
                ...
                if find('*', mitems[itmLabel][sf]) = 0 then --if not disabled
