...
wcpSizeModeY,   --1 = Chlldren will be the same size as the child with the largest Natural size, container will be minimum size
...
wcpAdjust,      --list of adjustments: {firstchild, secondchild, etc...} integer: size in pixels, (decimal 0.01 - 0.99): size in percentage of container's total size
...
procedure wc_create(atom wid, object wprops)
    ...
    sequence wpos, wsize
    object adj = {}, lbl = 0, margin = {0, 0, 0, 0}, spacing = 2
...
procedure wc_draw(atom wid)
    ...
    atom idx, bcolor, fcolor
    idx = find(wid, wcprops[wcpID])
    
    if idx > 0 then
        /*
        --containers don't need to draw anything, except for testing
        wrect = widget_get_rect(wid)
        fcolor = rgb(150, 0, 0) 
        cmds = {
            {DR_PenColor, fcolor},
            {DR_Rectangle, False} & wrect
        }
...
procedure wc_resize(atom wid)  --resizing affects parent and ancestors
    ...
    if idx > 0 then
        ...
        --determine container's size based on orientation, size mode, and children sizes 
        ...
        if not equal(widget:widget_get_class(widget:parent_of(wid)), "tabs") and sequence(wcprops[wcpGroupLabel][idx]) then
...
wc_define_command("container", "set_container_props", routine_id("cmd_set_props"))
...
wc_define_function("container", "get_box_width", routine_id("cmd_get_box_width"))
...
wc_define_function("container", "get_orientation", routine_id("cmd_get_orientation"))
