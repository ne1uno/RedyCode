...
procedure wc_create(atom wid, object wprops)
    ...
    wSubStartPos = 0, wSubEndPos = 0, wAddStartPos = 0, wAddEndPos = 0, wPressed = 0, wClicked = 0, wMovingOffset = 0, wAttachedWidget = 0, wStayAtBottom = 0
        ...
        if length(wprops[p]) = 2 then
            switch wprops[p][1] do
                ...
                case "orientation" then
                    wOrientation = wprops[p][2]
                
                case "attach" then
                    wAttachedWidget = wprops[p][2]
                
                case "stayatbottom" then
                    wStayAtBottom = wprops[p][2]
    ...
    wcprops[wcpStayAtBottom] &= {wStayAtBottom}
...
--sub button:          
        ...
        cmds &= {
            ...
            --fill triangle
            {DR_PenColor, scrArrowColor},
            {DR_BrushColor, scrArrowColor}
        }
        if wcprops[wcpOrientation][idx] = scrV then
            tx = floor(x1 + scrSize * 0.5)
            ty = floor(y1 + scrSize * 0.5) - 1
            cmds &= {
                {DR_PolyLine, True, {
                    {tx - ts, ty + ts},
                    {tx, ty - ts},
            ...
            tx = floor(x1 + scrSize * 0.5) - 1
            ty = floor(y1 + scrSize * 0.5)
            cmds &= {
                {DR_PolyLine, True, {
                    {tx + ts, ty - ts},
                    {tx - ts + 1, ty},
        ...
        if wcprops[wcpOrientation][idx] = scrV then
            cmds &= {
                {DR_Line, tx - ts, ty + ts, tx, ty - ts}
            }
        else
            cmds &= {
                {DR_Line, tx + ts, ty - ts, tx - ts, ty}
            }
        end if
        ...
        if wcprops[wcpOrientation][idx] = scrV then
            cmds &= {
                {DR_Line, tx - ts, ty + ts, tx + ts, ty + ts},
                {DR_Line, tx + ts, ty + ts, tx, ty - ts}
            }
        else
            cmds &= {
...
procedure wc_event(atom wid, sequence evtype, object evdata)
    ...
    if idx > 0 then
        ...
        switch evtype do        
            ...
            case "LeftUp" then      
                if wcprops[wcpClicked][idx] = 1 and wcprops[wcpPressed][idx] > 0 then
                    doredraw = 1
                end if
...
procedure wc_resize(atom wid)
    ...
    if idx > 0 then
        if wcprops[wcpOrientation][idx] = scrV then
            widget:widget_set_min_size(wid, scrSize, scrSize*2)
            widget:widget_set_natural_size(wid, scrSize, 0)
    
        else
            widget:widget_set_min_size(wid, scrSize*2, scrSize)
...
procedure wc_arrange(atom wid)
    ...
    if idx > 0 then
        ...
        if scrStyle = 0 then --0 = buttons on each end
            wcprops[wcpMinPos][idx] = scrSize
            wcprops[wcpMaxPos][idx] = wcprops[wcpLength][idx] - scrSize
            
            wcprops[wcpSubStartPos][idx] = 0
            wcprops[wcpSubEndPos][idx] = scrSize
            wcprops[wcpAddStartPos][idx] = wcprops[wcpLength][idx] - scrSize
...
wc_define_command("scrollbar", "set_length", routine_id("cmd_set_length"))
...
wc_define_command("scrollbar", "set_min", routine_id("cmd_set_min"))
...
wc_define_command("scrollbar", "set_max", routine_id("cmd_set_max"))
...
wc_define_command("scrollbar", "set_range", routine_id("cmd_set_range"))
...
wc_define_command("scrollbar", "set_value", routine_id("cmd_set_value"))
...
wc_define_command("scrollbar", "set_value_rel", routine_id("cmd_set_value_rel"))
