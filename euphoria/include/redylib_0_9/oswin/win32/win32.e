    ...
    switch iMsg do
        ...
        case WM_SIZE then    
            if widx > 0 then
                ...
                window_add_event(hwnd, "Resize", {params[3], params[4], params[1]})
                ...
                --window_add_event(hwnd, "Maximize", {1})
        ...
        case WM_DESTROY then
            if widx > 0 then
                ...
                if wowner = 1 then --or widx = 1 then
                    --if main window closes, then destroy all windows
                    for w = 1 to length(windowList) do
                        window_add_event(windowList[w], "Close", {})
                    end for
                end if
                if length(windowList) = 0 then --if no windows are open, then end process
        ...
        case WM_SETFOCUS then
            if MenuActive = 0 or equal(windowStyle[widx], "popup") then
                WindowFocus = hwnd
...
public function create_window(atom wid, sequence wtitle, object wstyle,
atom wleft, atom wtop, atom wwidth, atom wheight, atom wmaximized = 0, atom wbackcolor, atom wparent = 0)
    ...
    switch wstyle do
        case "normal" then
            if wmaximized then
                winstyle = or_all({WS_OVERLAPPEDWINDOW, WS_MAXIMIZE})
            else
        ...
        case "noborder" then
            if wmaximized then
                winstyle = or_all({WS_POPUP, WS_MAXIMIZE})
            else
        ...
        case "notitlebar" then
            if wmaximized then
                winstyle = or_all({WS_POPUP, WS_THICKFRAME, WS_MAXIMIZE})
            else
        ...
        case "popup" then
            winstyle = or_all({WS_POPUP})
            winexstyle = or_all({WS_EX_NOACTIVATE})
            winclass = "MenuWindow"
...
public procedure show_window(atom hWnd, atom maximized = 0)
    atom widx = find(hWnd, windowList), ptrRect 
    sequence rect
    
    if widx > 0 then
        if maximized then
            c_proc(xShowWindow, {hWnd, SW_SHOWMAXIMIZED})
        else
        ...
        free(ptrRect)
        window_add_event(hWnd, "Resize", {rect[3], rect[4], 0})
...
public procedure set_window_size(atom hWnd, atom width, atom height) --set window size by client size
    ...
    if widx > 0 then
        ...
        window_add_event(hWnd, "Resize", {width + diff[1], height + diff[2], 0})
    end if
end procedure
...
public function bitmap_effect(atom hwnd, sequence bitmapname, sequence effectname, atom refresh = 0)
    --create a new bitmap with applied effect and return new bitmap name (bitmapname & "_" & effectname)
    --if effect bitmap already exists, do not recreate it unless refresh = 1
    
    if not find(bitmapname, hBitmapNames) 
    or not find(effectname, {"greyscale", "disabled", "covered"}) then
        return  ""
    end if
    
    if not find(bitmapname & "_" & effectname, hBitmapNames) or refresh = 1 then
        sequence pxrgb,
        bitmapdata = bitmap_to_sequence(bitmapname, hwnd)
        atom pxcolor,
        DimX = length(bitmapdata[1]),
        DimY = length(bitmapdata)
        
        if equal(effectname, "greyscale") then  --convert color to greyscale
            for y = 1 to DimY do
                for x = 1 to DimX do
                    pxrgb = get_rgb(bitmapdata[y][x])
                    pxcolor = floor((pxrgb[1] + pxrgb[2] + pxrgb[3]) / 3)
                    bitmapdata[y][x] = rgb(pxcolor, pxcolor, pxcolor)
                end for
            end for
            
        elsif equal(effectname, "disabled") then
            for y = 1 to DimY do
                for x = 1 to DimX do
                    pxrgb = get_rgb(bitmapdata[y][x])
                    --convert color to greyscale
                    pxcolor = floor((pxrgb[1] + pxrgb[2] + pxrgb[3]) / 3)
                    --quantize grey shades
                    if pxcolor = 0 then
                        pxcolor = 0
                    elsif pxcolor < 128 then
                        pxcolor = 160
                    elsif pxcolor < 160 then
                        pxcolor = 176
                    elsif pxcolor < 192 then
                        pxcolor = 192
                    elsif pxcolor < 224 then
                        pxcolor = 208
                    else
                        pxcolor = 255
                    end if
        ...
        sequence_to_bitmap(hwnd, bitmapname & "_" & effectname, bitmapdata)
    end if
    
    return bitmapname & "_" & effectname
end function
            ...
            case DR_Image then --{hBitmap, x, y, optional transparency color}
                ...
                if bidx > 0 then
                    ...
                    */
                    ...
                    -- copy bitmap to device context
                    if length(ccmd) > 4 then --use transparent color
--BOOL TransparentBlt(HDC  hdcDest, int  xoriginDest, int  yoriginDest, int  wDest, int  hDest, HDC  hdcSrc,
--    int  xoriginSrc, int  yoriginSrc, int  wSrc, int  hSrc, UINT crTransparent);
                        VOID = c_func(xTransparentBlt, 
                            {hdc, xoff + ccmd[3], yoff + ccmd[4], bmWidth, bmHeight, bmDC, bmOrgX, bmOrgY, bmWidth, bmHeight, ccmd[5]})
                    else
--BitBlt(HDC hdcDest, int nXDest, int nYDest, int nWidth, int nHeight, HDC hdcSrc, int nXSrc, int nYSrc, DWORD dwRop);
